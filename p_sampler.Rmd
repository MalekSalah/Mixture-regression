...
for (i in 1:(T-1)){
  ...
  if ((i%%cur_adapt==0) && ((median(ac_rate_p[1:i])>.4) || (median(ac_rate_p[1:i])<.3))) {
    i_start <- i
    adapt_eps_p <- TRUE
  ...
  if (adapt_eps_p == TRUE){
    ...
    for (j in 1:T_adapt){
      ...
      if (j >= ac_rate_span){
        ...
        # Adjust the eps_p 
        if (current_ac_rate < ac_rate_lower) eps_p <- min(eps_p*1.4, 1000)
        else if (current_ac_rate > ac_rate_upper) eps_p <- max(0, eps_p*0.8)
      }
    }
    adapt_eps_p <- FALSE
  }
