...
## Sampling etaPhi and p with adaptation
...
for (i in 1:(T - 1)) {
  ...
  ##======================Sample the eta1, et2 and phi all squared========================>
    ...
    log_acceptance <-
      (
        Log_post_etaPhi(p_next, sqrt(etaPhi_sqrd_prop), sigma, mu, Alpha, Y) + ddirichlet(matrix(etaPhi_sqrd_t, nrow = 1), tmp1 , log = TRUE)
      ) - (Log_post_etaPhi(p_next, sqrt(etaPhi_sqrd_t), sigma, mu, Alpha, Y) + ddirichlet(matrix(etaPhi_sqrd_prop, nrow = 1), tmp2, log = TRUE))
    acc_etaPhi_sqrd <- min(1, exp(log_acceptance))
...
ggplot() +
  ...
  geom_vline(xintercept = sigma_1, color="red", show.legend = TRUE)  +
  geom_vline(xintercept = mean(sigma1), color="blue", show.legend = TRUE) +
  ggtitle(sprintf("Histogramme des echantillons de sigma1\nEn rouge la valeur exacte, en bleu l'estimateur de Bayes\nerr=%.6f", abs(mean(sigma1)-sigma_1)))
...
ggplot() +
  ...
  geom_vline(xintercept = sigma_2, color="red", show.legend = TRUE)  +
  geom_vline(xintercept = mean(sigma2), color="blue", show.legend = TRUE) +
  ggtitle(sprintf("Histogramme des echantillons de sigma2\nEn rouge la valeur exacte, en bleu l'estimateur de Bayes\nerr=%.6f", abs(mean(sigma2)-sigma_2)))
```
